...
int acpi_table_add(const char *t)
{
    ...
    if (!acpi_tables) {
        ...
        acpi_tables = calloc(1, allen);
    } else {
        allen = acpi_tables_len;
    }

    start = allen;
    acpi_tables = realloc(acpi_tables, start + ACPI_TABLE_HDR_SIZE);
    ...
    for (f = strtok(buf, ":"); f; f = strtok(NULL, ":")) {
        ...
        for (;;) {
            ...
            if (r == 0) {
                ...
                acpi_tables = realloc(acpi_tables, allen + r);
...
{
    gpe->len = len;
    gpe->sts = calloc(1, len / 2);
    gpe->en = calloc(1, len / 2);
}
