                  ...
                  void *irq_opaque, int nirq)
{
    ...
    bus->nirq = nirq;
    bus->irq_count = calloc(nirq, sizeof(bus->irq_count[0]));
}
...
static int pci_init_multifunction(PCIBus *bus, PCIDevice *dev)
{
    ...
    if (PCI_FUNC(dev->devfn)) {
        ...
        if (f0 && !(f0->cap_present & QEMU_PCI_CAP_MULTIFUNCTION)) {
            ...
            printf("PCI: single function device can't be populated "
    ...
    for (func = 1; func < PCI_FUNC_MAX; ++func) {
        if (bus->devices[PCI_DEVFN(slot, func)]) {
            printf("PCI: %x.0 indicates single function, "
...
static void pci_config_alloc(PCIDevice *pci_dev)
{
    ...
    pci_dev->config = calloc(1, config_size);
    pci_dev->cmask = calloc(1, config_size);
    pci_dev->wmask = calloc(1, config_size);
    pci_dev->w1cmask = calloc(1, config_size);
    pci_dev->used = calloc(1, config_size);
}
                                         ...
                                         const PCIDeviceInfo *info)
{
    ...
    if (devfn < 0) {
        ...
        printf("PCI: no slot/function available for %s, all in use", name);
        return NULL;
    found: ;
    } else if (bus->devices[devfn]) {
        printf("PCI: slot %d function %d not available for %s, in use by %s",
                               ...
                               PCIConfigWriteFunc *config_write)
{
    ...
    pci_dev = calloc(1, instance_size);
...
static int pci_unregister_device(DeviceState *dev)
{
    ...
    free(pci_dev->romfile);
    pci_dev->romfile = NULL;
...
static int pci_qdev_init(DeviceState *qdev, DeviceInfo *base)
{
    ...
    if (qdev->hotplugged && info->no_hotplug) {
        printf("QERR_DEVICE_NO_HOTPLUG, info->qdev.name:%s\n", info->qdev.name);
        do_pci_unregister_device(pci_dev);
        return -1;
    }
    if (info->init) {
    ...
    if (pci_dev->romfile == NULL && info->romfile != NULL) {
        pci_dev->romfile = strdup(info->romfile);
        is_default_rom = true;
    }
...
static int pci_add_option_rom(PCIDevice *pdev, bool is_default_rom)
{
    ...
    if (path == NULL) {
        path = strdup(pdev->romfile);
    }

    size = get_image_size(path);
    if (size < 0) {
        printf("%s: failed to find romfile \"%s\"",
                        ...
                        const char *default_devaddr)
{
    ...
    if (!bus) {
        printf("Invalid PCI device address %s for device %s",
                     devaddr, model);
        return NULL;
    }    

    pci_dev = pci_create(bus, devfn, model);
    dev = &pci_dev->qdev;
