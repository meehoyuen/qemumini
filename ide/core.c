                   ...
                   const char *version, const char *serial)
{
    ...
    if (cylinders < 1 || cylinders > 16383) {
        printf("cyls must be between 1 and 16383");
        return -1;
    }
    if (heads < 1 || heads > 16) {
        printf("heads must be between 1 and 16");
        return -1;
    }
    if (secs < 1 || secs > 63) {
        printf("secs must be between 1 and 63");
        return -1;
    }
    ...
    if (kind == IDE_CD) {
        ...
        if (!bdrv_is_inserted(s->bs)) {
            printf("Device needs media, but drive is empty");
            return -1;
        }
        if (bdrv_is_read_only(bs)) {
            printf("Can't use a read-only drive");
            return -1;
        }
    }
    if (serial) {
    ...
    if (version) {
        ...
        pstrcpy(s->version, sizeof(s->version), "1.0");
    }
    ...
    for(i = 0; i < 2; i++) {
        ...
        if (dinfo) {
            if (ide_init_drive(&bus->ifs[i], dinfo->bdrv,
                ...
                printf("Can't set up IDE drive %s", dinfo->id);
                exit(1);
            }
