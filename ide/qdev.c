...
static int ide_qdev_init(DeviceState *qdev, DeviceInfo *base)
{
    ...
    case 0:
        if (bus->master) {
            printf("IDE unit %d is in use", dev->unit);
            goto err;
        }
        bus->master = dev;
        break;
    case 1:
        ...
        break;
    default:
        printf("Invalid IDE unit %d", dev->unit);
        goto err;
    }
...
static int ide_dev_initfn(IDEDevice *dev, IDEDriveKind kind)
{
    ...
    if (dev->conf.discard_granularity && dev->conf.discard_granularity != 512) {
        printf("discard_granularity must be 512 for ide");
        return -1;
    }
    ...
    if (!dev->version) {
        dev->version = strdup(s->version);
    }
    if (!dev->serial) {
        dev->serial = strdup(s->drive_serial_str);
    }
