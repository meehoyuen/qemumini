...
{
    free(*(char **)qdev_get_prop_ptr(dev, prop));
    (*(char **)qdev_get_prop_ptr(dev, prop)) = NULL;
}
...
int qdev_prop_parse(DeviceState *dev, const char *name, const char *value)
{
    ...
    if (!prop || !prop->info->parse) {
        fprintf(stderr, "proproty not found: %s, %s\n", dev->info->name, name);
        return -1;
    }
    ...
    if (ret < 0) {
        switch (ret) {
        case -EEXIST:
            fprintf(stderr, "proproty value in use: %s, %s, %s\n", dev->info->name, name, value);
            break;
        default:
        case -EINVAL:
            fprintf(stderr, "proproty value bad: %s, %s, %s\n", dev->info->name, name, value);
            break;
        case -ENOENT:
            fprintf(stderr, "proproty value not found: %s, %s, %s\n", dev->info->name, name, value);
            break;
        }
        return -1;
    }
...
int qdev_prop_set_drive(DeviceState *dev, const char *name, BlockDriverState *value)
{
    ...
    if (res < 0) {
        printf("Can't attach drive %s to %s.%s: %s",
